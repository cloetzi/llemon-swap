...
func SendResponse(w http.ResponseWriter, r *http.Request, status int, message string) {
	...
	if strings.Contains(acceptHeader, "text/plain") {
		...
		w.Write([]byte(fmt.Sprintf("llemon-swap: %s", message)))
		return
	}
	...
	if strings.Contains(acceptHeader, "text/html") {
		...
		w.WriteHeader(status)
		w.Write([]byte(fmt.Sprintf(`<html><body><h1>llemon-swap</h1><p>%s</p></body></html>`, html.EscapeString(message))))
		return
	}

	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(status)
	resp, err := json.Marshal(map[string]string{"src": "llemon-swap", "error": message})
	if err != nil {
		w.Write([]byte(`{"src":"llemon-swap", "error": "failed to marshal response"}`))
		return
	}
	w.Write(resp)
}
